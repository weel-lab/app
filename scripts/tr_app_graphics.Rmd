...
theme_set(theme_gray(base_size = 25)) #theme_gray is the default theme
...
anpp_respiration_timeseries_plot <- ggplot(data = app_timeseries, aes(x = date)) + #calls ggplot and stores the call in a graphics object called "app_timeseries_plot"
  ...
  labs(x = "Date", y = "ANPP & Respiration (mg/L*hr)")+ #sets labels for each axes
  theme(legend.position=c(0.9,0.9))+ #positions read as (x,y), where x = 0 is left, y = 0 is bottom
  theme(legend.key=element_rect(fill='white'))+
  theme(legend.box.spacing=unit(0, "cm"))+
  theme(panel.background = element_rect(fill = 'white'))+
...
ggsave("tr_anpp_respiration_timeseries.eps", anpp_respiration_timeseries_plot, width = 12, path = '~/Google Drive/WEEL/Tres Rios/datasets/tr-aquatic-primary-productivity/graphics/')
